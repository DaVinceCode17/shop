...
public class CustomerDAO {
    ...
    public boolean saveCustomer(Customer customer) {
        String sql = "INSERT INTO customers (first_name, middle_initial, surname, address, contact_number, nickname) "
                + "VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public boolean updateCustomer(Customer customer) {
        String sql = "UPDATE customers SET first_name=?, middle_initial=?, surname=?, address=?, contact_number=?, nickname=? WHERE id=?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, customer.getFirstName());
            pstmt.setString(2, customer.getMiddleInitial());
